    ...
    <div className="relative min-h-screen bg-background">
      ...
      <header className="mx-auto flex max-w-5xl items-center justify-between px-6 pt-6">
        ...
        <nav className="text-sm">
          ...
          <Link to="/login" className="px-3 py-2 text-muted-foreground hover:text-foreground">{t("loginWord")}</Link>
        </nav>
      </header>

      <main className="mx-auto max-w-5xl px-6 py-12">
        <h1 className="font-display text-4xl font-semibold tracking-tight sm:text-5xl">{t("blogTitle")}</h1>
        <p className="mt-2 text-muted-foreground">{t("blogSub")}</p>
            ...
            >
              ...
              <div className="p-5">
                ...
                <p className="mt-3 text-[11px] uppercase tracking-wider text-muted-foreground">
                  {p.published_at && fmt.date(p.published_at, { day: "numeric", month: "long", year: "numeric" })}
                </p>
              </div>
            </Link>
          ))}
